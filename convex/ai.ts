...
Remember: You are a supportive coach, not a licensed therapist.`;
    ...
    if (args.userInfo?.userId) {
      ...
      if (moodInsights.recentMoods.length > 0) {
        const avgMood = moodInsights.recentMoods.reduce((sum: number, m: any) => sum + m.rating, 0) / moodInsights.recentMoods.length;
        contextualInfo = args.language === "ar"
          ? `\n\nمعلومات سياقية: متوسط مزاج المستخدم مؤخراً ${avgMood.toFixed(1)}/10.`
          : `\n\nContextual info: User's recent average mood is ${avgMood.toFixed(1)}/10.`;

        if (moodInsights.commonLowMoodTriggers.length > 0) {
          contextualInfo += args.language === "ar"
            ? ` محفزات المزاج المنخفض الشائعة: ${moodInsights.commonLowMoodTriggers.join("، ")}.`
            : ` Common low mood triggers: ${moodInsights.commonLowMoodTriggers.join(", ")}.`;
        }
      }
    }
